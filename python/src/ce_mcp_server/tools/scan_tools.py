...
def register(server: FastMCP, ctx: ToolContext) -> None:
    specs = [
        ...
        runtime_tool(ctx, name="ce.scan_get_state", description="Return structured state for one Cheat Engine memscan session, including whether a scan is running or results are ready.", runtime=SCAN_RUNTIME, function_name="get_session_state", parameters=(SCAN_SESSION_PARAMETER,)),
        ...
        runtime_tool(ctx, name="ce.scan_first", description="Start a first scan using Cheat Engine scan semantics.", runtime=SCAN_RUNTIME, function_name="first_scan", parameters=(SCAN_SESSION_PARAMETER, SCAN_OPTIONS_PARAMETER), timeout_seconds=120.0),
        runtime_tool(ctx, name="ce.scan_next", description="Start a next scan using Cheat Engine scan semantics.", runtime=SCAN_RUNTIME, function_name="next_scan", parameters=(SCAN_SESSION_PARAMETER, SCAN_OPTIONS_PARAMETER), timeout_seconds=120.0),
        runtime_tool(ctx, name="ce.scan_wait", description="Wait for an active scan to finish.", runtime=SCAN_RUNTIME, function_name="wait", parameters=(SCAN_SESSION_PARAMETER,), timeout_seconds=180.0),
