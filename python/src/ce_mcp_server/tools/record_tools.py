...
def register(server: FastMCP, ctx: ToolContext) -> None:
    specs = [
        ...
        runtime_tool(ctx, name="ce.record_create_many", description="Create multiple top-level cheat table records in one call.", runtime=TABLE_RUNTIME, function_name="create_records", parameters=(ParameterSpec("records", list[dict[str, object]]),)),
        runtime_tool(ctx, name="ce.record_create_group", description="Create a grouped cheat table header and append child records under it.", runtime=TABLE_RUNTIME, function_name="create_record_group", parameters=(ParameterSpec("description", str), ParameterSpec("records", list[dict[str, object]]), ParameterSpec("options", dict[str, object], {}))),
