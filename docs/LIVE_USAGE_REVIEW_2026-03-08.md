...
This document records the real issues found during live use against `Tic-tak-toe.exe`, and the changes shipped in `0.2.4`, `0.2.5`, `0.2.6`, and `0.2.7` to close them.
...
This review is now the shipped-fix record for the `0.2.4` and `0.2.5` releases. The Minecraft-focused follow-up for `0.2.6` is documented in `docs/MINECRAFT_LIVE_AUDIT_2026-03-08.md`, and the libhat signature-scan rewrite follow-up for `0.2.7` is documented in `docs/LIBHAT_SIGSCAN_REWRITE_2026-03-08.md`.
